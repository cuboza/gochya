...
Shared Core подключён к телефону через `dart:ffi`: мост сверяет ABI при
открытии, штампует versioned-заголовки структур и переводит отказ ядра в
исключение, не подменяя его значением по умолчанию. В этом срезе через FFI идёт
уход (`advance_needs`, `apply_care`) — как локальное предсказание между чтениями
профиля; бой, геном и экономика остаются серверными. Тесты моста работают против
настоящей библиотеки, которую собирает `tools/build-core-host.sh`.
