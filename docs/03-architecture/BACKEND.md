...
участников. Вместе с раундами он отдаёт `elementA`/`elementB` — виды обеих
сторон, прочитанные из неизменяемого snapshot лоадаута, поэтому клиент рисует
именно тех существ, которых симулировал сервер, даже если питомец позже сменил
стихию. Приватное состояние соперника (needs, статы, инвентарь) в replay не
попадает. Миграция `000008_casual_matches` создаёт `matches` и отдельную
