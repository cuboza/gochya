...
- Клиент сохраняет журнал команд с `operation_id` и `base_revision`, а не загружает готовое состояние.
- При восстановлении соединения сервер дедуплицирует и повторно исполняет допустимые команды через Shared Core (`OFFLINE_SYNC.md`).
...
- Нормативный протокол и конфликтные сценарии: `OFFLINE_SYNC.md`.
...
- `docs/04-core/CORE_ABI.md` — нормативная C ABI граница.
- `OFFLINE_SYNC.md` — журнал офлайн-команд и reconciliation.
- `HEALTH_DATA_CONTRACT.md` — source-of-truth и дедупликация health records.
