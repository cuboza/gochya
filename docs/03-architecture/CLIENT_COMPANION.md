...
Реализован: `features/dojo_upsell/`. Два пути к картам показаны рядом с их
потолками редкости (Epic против Mythic) в цветах шкалы §3.3 арт-библии, дальше
блок «Ты уже конкурентоспособен» и CTA. Вход — в конце коллекции на экране
боевых карт: именно там игрок задаётся вопросом, откуда берутся карты получше.

**Кнопки «Подключить» намеренно нет.** Спаривания с часами в клиенте не
существует, и кнопка, которая ничего не делает, была бы хуже честной фразы:
карточка объясняет, что установить GOCHYA на часы нужно тем же аккаунтом.
Тесты стерегут тон — ни одного выключенного контрола на экране и ни одного
слова вроде «Купить», «Осталось», «Недоступно».
