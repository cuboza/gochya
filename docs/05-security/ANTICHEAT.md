...
- Телефон сериализует параллельные refresh в single-flight, атомарно заменяет
  access/refresh pair одним защищённым документом и повторяет исходный API-запрос
  не более одного раза. Неопределённый refresh-ответ fail-closed завершает
