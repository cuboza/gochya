...
```
ELEMENT_TABLE (полная — в BALANCE.md):
    Fire > Earth > Air > Water > Fire    (классический круг)
    Light <> Dark (взаимно ×2)
    Arcane: 1.2 ко всем, но 0.8 от всех
    Гибриды: среднее от родителей

elementMultiplier(atk, def):
    if atk == def: 1.0
    if atk сильнее def: 1.5
    if atk слабее def: 0.67
    else: 1.0
```
