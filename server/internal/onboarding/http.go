...
func NewHTTPHandler(
	...
	if service == nil || authenticator == nil {
		return nil, errors.New("onboarding service and authenticator are required")
	}
	if random == nil {
		random = rand.Reader
	}
...
func (h *HTTPHandler) Routes() http.Handler {
	...
	mux.HandleFunc("/v1/me/onboarding/age-gate", h.handleAgeGate)
	mux.HandleFunc("/v1/me/onboarding/starter-egg", h.handleStarterEgg)
	return mux
}

func (h *HTTPHandler) handleAgeGate(writer http.ResponseWriter, request *http.Request) {
	...
	request.Body = http.MaxBytesReader(writer, request.Body, maxOnboardingBody)
	var payload AgeGateRequest
	if err := decodeStrictJSON(request.Body, &payload); err != nil {
		h.writeError(writer, requestID, err)
		return
	}
	response, err := h.service.RecordAgeGate(
		request.Context(),
		playerID,
...
func (h *HTTPHandler) handleStarterEgg(
	...
	request.Body = http.MaxBytesReader(writer, request.Body, maxOnboardingBody)
	var payload StarterEggRequest
	if err := decodeStrictJSON(request.Body, &payload); err != nil {
		h.writeError(writer, requestID, err)
		return
	}
	response, err := h.service.SelectStarterEgg(
		request.Context(),
		playerID,
		request.Header.Get("Idempotency-Key"),
		payload,
