...
func NewHTTPHandler(
	...
	if service == nil || authenticator == nil {
		return nil, errors.New("care service and authenticator are required")
	}
	if random == nil {
		random = rand.Reader
	}
...
func (h *HTTPHandler) Routes() http.Handler {
	...
	mux.HandleFunc("/v1/sync/commands", h.handleSync)
	return mux
}

func (h *HTTPHandler) handleSync(writer http.ResponseWriter, request *http.Request) {
	...
	request.Body = http.MaxBytesReader(writer, request.Body, maxSyncBody)
	var payload SyncRequest
	if err := decodeStrictJSON(request.Body, &payload); err != nil {
		h.writeError(writer, requestID, err)
		return
	}
	response, err := h.service.Sync(
		request.Context(),
		playerID,
